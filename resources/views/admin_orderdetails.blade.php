...
table th {
   ...
   <body>   
      ...
      <table>
        ...
        @foreach($checkout2 as $check)
            <tr>
                <td>{{ $check->user->name }}</td>
                <td>{{ $check->getUser->name }}</td>
                <td>{{ $check->post->book_title }}</td>
                <td>{{ $check->post->phone_number }}</td>
                <td>{{ $check->phone_number }}</td>
                <td>
                    <img src="{{ asset('storage/images/' . $check->post->image) }}" alt="Book Image" width="100" height="100">
                </td>
            </tr>
        @endforeach
    </table>
