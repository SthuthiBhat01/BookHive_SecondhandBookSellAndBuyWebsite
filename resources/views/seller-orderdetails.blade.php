...
<body>
    ...
    <h1 class="booksold">Orders From Buyers </h1>
    <h5>View a list of the books you got order from our platform.</h5>
    ...
    <table>
        ...
        @foreach($buyerInfo as $buyer)
            <tr>
                ...
                <td>{{ $buyer['book_title'] }}</td>
                <td>{{ $buyer['book_price'] }}</td>
                <td>{{ $buyer['buyer_name'] }}</td>
                <td>{{ $buyer['buyer_phone'] }}</td>
                <td>
                    <img src="{{ asset('storage/images/' . $buyer['book_image']) }}" alt="Book Image" width="100" height="100">
                </td>
            </tr>
        @endforeach
    </table>
